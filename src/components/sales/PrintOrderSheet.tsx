...
import { salesOrderToPdfData } from "@/lib/partyDocumentPdfBuilder";
...
export function PrintOrderSheet({ open, onOpenChange, order }: PrintOrderSheetProps) {
    ...
    const pdfData = useMemo(
        () => (order ? salesOrderToPdfData(order) : null),
        [order],
    );
    ...
    const totalAmount = order.items
        ? order.items.reduce((sum, i) => sum + Number(i.qty || 0) * Number(i.rate || 0), 0)
        : 0;

    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto p-0 gap-0">
                <DialogTitle className="sr-only">Sales order {order.id}</DialogTitle>
                    ...
                    >
                        <div className="flex justify-between items-start border-b pb-6 mb-6">
                            ...
                            <div className="text-right">
                                <h2 className="text-lg font-bold text-slate-900 uppercase">Sales Order</h2>
                                <p className="text-sm text-slate-500 mt-1">Party copy</p>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
                            <div>
                                ...
                                <div className="text-slate-900 font-semibold">{order.customer}</div>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-8 mt-8 pt-8 border-t border-slate-200">
                            <p className="text-xs text-slate-500">
                                This sales order is shared with the customer for confirmation. It is not a tax invoice.
                            </p>
